...
:END:
The web canvas is a serious surface, pressure/tilt ink,
pen/marker/pencil/ highlighter/eraser, color and width,
line/rectangle/ellipse/arrow shape tools, undo/redo, pan and pinch-zoom,
and a light/dark toggle, that runs on anything with a browser (iPad
Safari, Android, desktop), no iPad app or developer account needed.
...
Tested primarily on macOS, but Linux works too: the mDNS advertisement uses =avahi-publish= (install =avahi-utils= and run =avahi-daemon=), and everything else is platform-agnostic. Either way, prefer the =.local= setup URL — it resolves via mDNS to the machine's current IP, so it keeps working even when the IP changes, and the native app auto-reconnects by re-finding the server by name if it does.
