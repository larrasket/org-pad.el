...
org-pad lets you draw on your iPad with Apple Pencil and have the result
 in your org-mode buffer as an inline figure.

Emacs runs a small, pure-Elisp HTTP server; a companion iPad app (built with PencilKit, running inside Swift Playgrounds) long-polls that server for drawing requests, lets you sketch, and uploads the result back. The PNG that comes back is self-contained.
...
I have only tested this on macOS, but Bonjour should do in Linux too.
